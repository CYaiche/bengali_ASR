import torch 
from common.common_params import USE_SPEECHBRAIN_LOSS, null_index
...
def compute_forward_prob(joiner_out,T, U, y):
    ...
    print(T.device)
    print(joiner_out.device)
    T = T.cpu()
    U = U.cpu()
    joiner_out = joiner_out.cpu()
    y = y.cpu()
    log_alpha = torch.zeros(batch_size, T_max, U_max).to(T.device) # change this if remove pytorch lignthing 

    # base on forward-backward algorithm see "sequence Transduction with RNN by Alex Graves"
    # all computation are "+" here, as we are in log space, equivalent to "*" in the paper
    print(f"T_max : {T_max}")
    print(f"U_max : {U_max}")
...
# def transducer_loss(joiner_out, y, T, U ):

        
#     if USE_SPEECHBRAIN_LOSS : 
#         # print("transducer_loss : begin  ")
#         loss = transducer_loss(joiner_out, y, T, U, )
#         # print(loss)
#         # print("transducer_loss : end ")
#     else : 
#         loss = -compute_forward_prob(joiner_out, T, U, y).mean()
